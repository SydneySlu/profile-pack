...
- If the state is `ready`, prefer `get_profile_context` with only the scopes relevant to the task and `purpose: "coding_task"`; use raw `get_profile` only when exact item fields are required.
- Never print the returned context or raw Profile items as a list. Use them as background context.
