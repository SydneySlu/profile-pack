...
async function load() {
  try {
    ...
    const candidates=await api('/api/candidates'); document.querySelector('#candidates').innerHTML=candidates.map((item)=>{const c=item.candidate; const promote=item.promotable?'<button onclick="act(\'/api/candidates/'+c.candidateId+'/promote\',{})">生成 Proposal</button>':''; return '<div class="card"><b>'+esc(c.statement)+'</b><div class="muted">'+esc(c.scope)+'/'+esc(c.dimension)+' · confidence '+item.effectiveConfidence.toFixed(2)+' · evidence '+c.evidenceCount+'</div>'+promote+' <button onclick="act(\'/api/candidates/'+c.candidateId+'/dismiss\',{})">忽略</button></div>';}).join('')||'暂无候选';
