...
async function rollbackVersion(){const v=prompt('请输入要恢复的版本号');if(v){await act('/api/versions/rollback',{version:Number(v)})}}
async function load(){
 try{document.querySelector('#status').textContent=JSON.stringify(await api('/api/status'),null,2);document.querySelector('#profile').textContent=JSON.stringify(await api('/api/profile'),null,2);document.querySelector('#versions').textContent=JSON.stringify(await api('/api/versions'),null,2);
