...
async function main(): Promise<void> {
  ...
  if (command === "agents") { console.log(JSON.stringify(await store.listAgentPolicies(), null, 2)); return; }
  if (command === "authorize-sensitive") { const agentId = option("--agent-id"); const itemIds = option("--item-ids", "")!.split(",").map((item) => item.trim()).filter(Boolean); if (!agentId) throw new Error("Usage: profile-pack authorize-sensitive --agent-id codex --item-ids id1,id2"); console.log(JSON.stringify(await store.setAgentSensitiveItems(agentId, itemIds), null, 2)); return; }
  if (command === "promote") {
