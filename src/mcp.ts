...
export function createServer(profileStore: ProfileStore, learning = new LearningEngine(profileStore.rootDir)): McpServer {
  ...
  server.registerTool("extract_profile_observations", { description: "Use the configured OpenAI-compatible or local Ollama model to extract conservative candidate observations from a conversation. Raw transcript is not stored.", inputSchema: { transcript: z.string().min(1), scope: z.string().default("global"), agentId: z.string().default("unknown") } }, async ({ transcript, scope, agentId }) => { const extractor = createExtractorFromEnv(); const extracted = await extractor.extract(transcript, scope); const results = []; for (const observation of extracted) results.push(await learning.observe(observationsToInput(observation, agentId))); return result({ extractedCount: extracted.length, results }); });
