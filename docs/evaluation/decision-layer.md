...
The `DecisionEvaluator` interface and `RuleEvaluator` baseline are in place; a Jev adapter and API calls are not. Jev should be integrated after several low-risk decisions have been exercised and we have a small, fixed evaluation set with candidate options and the user's eventual choice/feedback. The next Jev phase can then add an opt-in adapter and compare its result with the rule baseline in shadow mode. Before any real request, the user must configure an API key and explicitly enable remote evaluation; only a minimized decision goal, candidate summaries, and required constraints should be sent, never the full Profile or Project Profile by default. Evaluate Chinese behavior, latency, cost/limits, timeout/failure handling, and agreement with user choices. Jev must not become the default until the comparison demonstrates practical value. The existing rule evaluator remains the no-cost, no-network default and fallback.
